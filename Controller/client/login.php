...
class Login
{
    ...
    {
        $username = $password = $fullName = NULL; // gán  $username = $password = $fullName = rỗng
        $error = array();
        ...
        if (!empty($_POST['login_admin'])) {
            ...
            if ($username && $password) {
                $result = $userModel->login($username, $password); // kiểm tra tài khoản mật khẩu
            ...
            */
                if ($check > 0) {
                    ...
                    if ($data['level'] == 1) { //Nếu level = 1 thì chuyển hướng đến trang quản trị viên
                        header('Location: View/admin');
                    } else {
                        echo "<script>alert('Vui lòng đăng nhập lại')</script>"; //Ngược lại thì thông báo đăng nhập lại
                    }
                } else {
